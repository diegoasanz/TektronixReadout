...
class CalibrationRunsCAEN:
	...
	def SetupDigitiser(self):
		...
		rfile.write('[COMMON]')
		rfile.write('# open the digitezer')
		rfile.write('OPEN PCI {ol} {n} {ba}'.format(ol=int(self.optlink), n=int(self.node), ba=int(self.vme_b_addr)))
		rfile.write('# GNUPLOT path, normally /usr/bin/')
		rfile.write('GNUPLOT_PATH\t"/usr/bin/"')
		rfile.write('# output format can be BINARY or ASCII')
		rfile.write('OUTPUT_FILE_FORMAT\tBINARY')
		rfile.write('# if OUTPUT_FILE_HEADER is YES, the structure of the event has to be changed in self.struct_fmt to include the header')
		rfile.write('OUTPUT_FILE_HEADER NO')
		rfile.write('# specify the amount of samples to save. This defines the event window')
		rfile.write('RECORD_LENGTH\t{p}'.format(p=int(self.points)))
		rfile.write('# number of events to save in the file')
		rfile.write('MAX_NUM_EVENTS\t{n}'.format(n=int(self.meas)))
		rfile.write('TEST_PATTERN\tNO')
		rfile.write('ENABLE_DES_MODE\tNO')
		rfile.write('# use external trigger. Options are: DISABLED, ACQUISITION_ONLY, ACQUISITION_AND_TRGOUT')
		rfile.write('EXTERNAL_TRIGGER\tDISABLED')
		rfile.write('# specify maximum number of events to read out in one Block Transfer. Must be between 1 and 1023')
		rfile.write('MAX_NUM_EVENTS_BLT\t100')
		rfile.write('# the percentage of the amount of data stored after the trigger in the event window. Has an offset of ~1.6... only accepts integers')
		rfile.write('POST_TRIGGER\t{pt}'.format(pt=int(round(self.post_trig_percent*0.9996 - 1.6384))))
		rfile.write('# number of events that have to be ready before readout when the IRQ is asserted. 0 means run continuously. 1023 is the maximum')
		rfile.write('USE_INTERRUPT\t0')
		rfile.write('# type of the fornt panel LEMO connectors: NIM, TTL')
		rfile.write('FPIO_LEVEL\tNIM')
		rfile.write('SKIP_STARTUP_CALIBRATION\tNO')

		sig_polarity = 'POSITIVE' if self.bias >= 0 else 'NEGATIVE'

		rfile.write('# configuration for each channel [0] to [7]')
		for ch in xrange(8):
			rfile.write('[{ch}]'.format(ch=ch))
			if ch == self.sigCh or ch == self.trigCh:
				rfile.write('ENABLE_INPUT\tYES')
			else:
				rfile.write('ENABLE_INPUT\tNO')
			if ch == self.sigCh:
				rfile.write('PULSE_POLARITY\t{sp}'.format(sp=sig_polarity))
				rfile.write('DC_OFFSET\t{o}'.format(o=self.sig_offset))
				rfile.write('CHANNEL_TRIGGER\tDISABLED')
			if ch == self.trigCh:
				rfile.write('PULSE_POLARITY\tPOSITIVE')
				rfile.write('DC_OFFSET\t{o}'.format(o=self.trig_offset))
				rfile.write('CHANNEL_TRIGGER\tACQUISITION_ONLY')
				rfile.write('TRIGGER_THRESHOLD\t{th}'.format(th=int(round(np.divide(self.trigVal+1-self.trig_offset/50.0, self.sigRes)))))
		rfile.close()
		print 'Done'
